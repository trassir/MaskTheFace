...
class FaceMaskTransformation(ImageOnlyTransform):
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        if not with_keypoints:
            for (x1, y1, x2, y2) in params[self._bboxes_key]:
            ...
            for (x1, y1, x2, y2), keypoints in zip(params[self._bboxes_key], params[self._keypoints_key]):
                x1, y1, x2, y2 = map(int, (x1 * w, y1 * h, x2 * w, y2 * h))
                keypoints *= [[w, h]]
                bboxes.append([x1, y1, x2, y2])
                kps_by_bbox.append(keypoints.astype(np.long))
